...
import { addProductAdminAction, getProductListAction } from '../../../redux/actions';
import { useEffect, useState } from 'react';

function AdminProduct({ productList, addProduct, getProduct}){
  ...
  function renderProduct() {
    return productList.data.map((item) => {
      return <Item
      id={item.id} key={item.id} name={item.name} price={item.price} 
      />
    })
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    addProduct: (params) => dispatch(addProductAdminAction(params)),
